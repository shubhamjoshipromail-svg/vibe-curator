 ...
 *
 * This is also the only file in the project allowed to import `ffmpeg-static`.
 * Anything under `src/` is bundled for the browser, and Vite would try to walk
 * a native binary into the client build.
 */
...
function ffmpegBinary(): string {
  // The one failure that is not an audio problem, so the one that may throw.
  if (!ffmpegPath) throw new Error('ffmpeg is unavailable: ffmpeg-static did not resolve a binary for this platform.');
  return ffmpegPath;
}
